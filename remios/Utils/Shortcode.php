...
/**
     ...
     *
     * self::load->library('shortcode');
     * self::shortcode->add('shortcode', 'myCodeParser');
     ...
     *
     * $out = self::shortcode->run($content);
     */
...
class Shortcode {
     ...
     * 	return "foo = {$atts[foo]}";
     * }
     * self::shortcode->add('footag', 'footag_func');
     * </code>
     *
     ...
     * 	return "foo = {$foo}";
     * }
     * self::shortcode->add('bartag', 'bartag_func');
     * </code>
     *
     ...
     * }
     * self::shortcode->add('baztag', 'baztag_func');
     * </code>
     *
     * @uses $shortcode_tags
     *
     ...
     */
    public static function run($content, $ignore_html="") {
        ...
        $content = preg_replace_callback('/' . $pattern . '/s', 'self::do_shortcode_tag', self::ClearCharter($content));
        $content = self::unescape_invalid_shortcodes( $content );
        
        return $content;
    }

    public static function ClearCharter($content){
        $content = @preg_replace( '%<p>&nbsp;\s*</p>%', '', $content ); // Remove all instances of "<p>&nbsp;</p>" to avoid extra lines.
        //$Old     = array( '<br />', '<br>' );
        //$New     = array( '','' );
        //$content = str_replace( $Old, $New, $content );
        $content = @preg_replace("/(<p>)?\[([\s[^\]]+])(\s[^\]]+)?\](.*)(<\/p>|<br \/>)?/","[$2$3]$4[/$2]",$content);
    ...
    public static function unescape_invalid_shortcodes($content ) {
...
// self-closing tag
     ...
     */
public static function do_shortcodes_in_html_tags( $content, $ignore_html, $tagnames ) {
         ...
         foreach ( $textarr as &$element ) {
                 ...
                 $attributes = self::kses_attr_parse( $element );
                 if ( false === $attributes ) {
                 ...
                 foreach ( $attributes as &$attr ) {
                         ...
                         if ( ( false === $single || $open < $single ) && ( false === $double || $open < $double ) ) {
                                 ...
                                 $count = 0;
                                 $new_attr = preg_replace_callback( "/$pattern/", 'self::do_shortcode_tag', $attr, -1, $count );
...
public static function kses_attr_parse( $element ) {
...
public static function kses_hair_parse( $attr ) {
    if ( '' === $attr ) {
        return array();
    }
