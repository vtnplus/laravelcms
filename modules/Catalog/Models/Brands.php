...
class Brands extends DBQuery{
	protected $table = 'brands';
	public $timestamps = false;
	
	public function links(){
        if(config("register.router.".$this->type)){
            return base_url(config("register.router.".$this->type)."/brands").($this->seo_urls ? "/".$this->seo_urls : '?pid='.$this->id);
        }else if(config("register.pages.urls") == $this->type){
            return base_url("pages/".config("register.pages.urls")."/brands").($this->seo_urls ? "/".$this->seo_urls : '?pid='.$this->id);
        }else{
            return base_url("brands").($this->seo_urls ? "/".$this->seo_urls : '?pid='.$this->id);
        }
